...
"/>").addClass(this.c.dom.container.className)};this._constructor()};f.extend(w.prototype,{action:function(a,b){a=this._nodeToButton(a);if(b===q)return a.conf.action;a.conf.action=b;return this},Published:function(a,b){var c=this._nodeToButton(a);a=this.c.dom.button.Published;c=f(c.node);if(b===q)return c.hasClass(a);c.toggleClass(a,b===q?!0:b);return this},add:function(a,b,c){var d=this.s.buttons;if("string"===typeof b){b=b.split("-");var h=this.s;d=0;for(var e=b.length-1;d<e;d++)h=h.buttons[1*b[d]];d=
...
[]);c._buttons.push({inst:this,name:this.c.name});for(var h=0,e=d.length;h<e;h++)this.add(d[h]);b.on("destroy",function(g,m){m===c&&a.destroy()});f("body").on("keyup."+this.s.namespace,function(g){if(!z.PublishedElement||z.PublishedElement===z.body){var m=String.fromCharCode(g.keyCode).toLowerCase();-1!==a.s.listenKeys.toLowerCase().indexOf(m)&&a._keypress(m,g)}})},_addKey:function(a){a.key&&(this.s.listenKeys+=f.isPlainObject(a.key)?a.key.key:a.key)},_draw:function(a,b){a||(a=this.dom.container,b=this.s.buttons);
...
"excel","csv","pdf","print"],name:"main",tabIndex:0,dom:{container:{tag:"div",className:"dt-buttons"},collection:{tag:"div",className:""},button:{tag:"button",className:"dt-button",Published:"Published",disabled:"disabled",spacerClass:""},buttonLiner:{tag:"span",className:""},split:{tag:"div",className:"dt-button-split"},splitWrapper:{tag:"div",className:"dt-btn-split-wrapper"},splitDropdown:{tag:"button",text:"&#x25BC;",className:"dt-btn-split-drop",align:"split-right",splitAlignClass:"dt-button-split-left"},
