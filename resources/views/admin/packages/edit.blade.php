...
@section('content')
    ...
    {{-- External CSS --}}
    <link href="{{ asset('Back_end/css/packages/edit-package.css') }}" rel="stylesheet">
    ...
    <div class="admin-page-wrapper">
        <div class="card shadow-sm admin-page-card-flush">
            ...
            <div
                ...
                <div>
                    <h5 class="panel-title mb-0">Edit Package</h5>
                    <small class="text-muted">Update existing lab package</small>
                </div>
            ...
            <div class="card-body card-body-soft">
                ...
                @php
                    $isSpecial = (bool) old('is_special', $package->is_special ?? false);
                    $oldStatus = old('status', $package->status ?? null);
                @endphp

                <form action="{{ route('admin.packages.update', $package->id) }}" method="POST"
                    enctype="multipart/form-data" id="packageEditForm" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="description" name="content" rows="5" required
                            class="ckeditor form-control wc-control @error('content') is-invalid @enderror" placeholder="Enter detailed description">{{ old('content', $package->content ?? '') }}</textarea>
                        @error('content')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea id="meta_tags" name="meta_tags" rows="5" required
                            class="form-control wc-control @error('meta_tags') is-invalid @enderror" placeholder="Enter advanced SEO meta tags">{{ old('meta_tags', $package->meta_tags ?? '') }}</textarea>
                        @error('meta_tags')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input id="discounted_price" type="number" step="0.01" name="discounted_price"
                                ...
                                placeholder="₹" value="{{ old('discounted_price', $package->discounted_price ?? '') }}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input id="banner" type="file" name="banner" accept="image/png, image/jpeg, image/webp"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input id="image_alt" type="text" name="image_alt"
                                ...
                                placeholder="Enter image alternative description"
                    ...
                    <div class="row mt-3 g-3">
                        @if (!empty($package->banner) && file_exists(public_path('storage/' . $package->banner)))
                            <div class="col-md-6">
                                <div id="currentBannerContainer" class="p-3">
                                    <label class="wc-field-label" style="font-size:0.9rem;">Current Image</label>
                                    <img id="currentBanner" src="{{ asset('storage/' . $package->banner) }}"
                                        alt="Current banner">
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="col-md-6">
                            <div id="bannerPreviewContainer" class="p-3" style="display:none;">
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input id="package_code" type="text" name="package_code"
                                ...
                                placeholder="e.g. PKG001" value="{{ old('package_code', $package->package_code ?? '') }}">
                            @error('package_code')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-3 mb-3">
                            ...
                            <input id="sample_type" type="text" name="sample_type"
                                ...
                                placeholder="e.g. Blood, Urine" value="{{ old('sample_type', $package->sample_type ?? '') }}">
                            @error('sample_type')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-3 mb-3">
                            ...
                            <input id="fasting" type="text" name="fasting"
                                ...
                                placeholder="e.g. Yes, No" value="{{ old('fasting', $package->fasting ?? '') }}">
                            @error('fasting')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-3 mb-3">
                            ...
                            <input id="parameters_count" type="number" name="parameters_count"
                                ...
                                placeholder="e.g. 55" value="{{ old('parameters_count', $package->parameters_count ?? '') }}">
                                ...
                                @foreach($tests as $t)
                                    <div class="form-check mb-1 test-item-row" data-name="{{ strtolower($t->test_name) }}">
                                        <input class="form-check-input" type="checkbox" name="test_ids[]" value="{{ $t->id }}" id="test_{{ $t->id }}"
                                            {{ (isset($selectedTestIds) && in_array($t->id, $selectedTestIds)) || (is_array(old('test_ids')) && in_array($t->id, old('test_ids'))) ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea id="why_done" name="why_done" rows="3"
                                ...
                                placeholder="Describe why this package is done">{{ old('why_done', $package->why_done ?? '') }}</textarea>
                            @error('why_done')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea id="who_should_test" name="who_should_test" rows="3"
                                ...
                                placeholder="Who should choose this package?">{{ old('who_should_test', $package->who_should_test ?? '') }}</textarea>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea id="how_to_read" name="how_to_read" rows="3"
                                ...
                                placeholder="How to interpret report results?">{{ old('how_to_read', $package->how_to_read ?? '') }}</textarea>
                            @error('how_to_read')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea id="what_to_ask" name="what_to_ask" rows="3"
                                ...
                                placeholder="Preparation required?">{{ old('what_to_ask', $package->what_to_ask ?? '') }}</textarea>
                            @error('what_to_ask')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input id="meta_title" type="text" name="meta_title"
                                ...
                                placeholder="Enter SEO Meta Title" value="{{ old('meta_title', $package->meta_title ?? '') }}">
                            @error('meta_title')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea id="meta_description" name="meta_description" rows="2"
                                ...
                                placeholder="Enter SEO Meta Description">{{ old('meta_description', $package->meta_description ?? '') }}</textarea>
                    ...
                    <div class="mb-3">
                        <div class="special-area">
                            <div>
                                <label class="special-toggle-pill {{ $isSpecial ? 'active' : '' }}"
                                    ...
                                    <input type="checkbox" id="is_special" name="is_special" value="1"
                                        {{ $isSpecial ? 'checked' : '' }} style="display:none;">
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div style="border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 16px; background: #fff;">
                            <div id="faqContainer">
                                {{-- Loop through existing or old input if validation failed --}}
                                @php
                                    $existingFaqs = old('faqs', $package->faqs ?? []);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    {{-- External JS --}}
    <script src="{{ asset('Back_end/js/packages/edit-package.js') }}"></script>
@endsection
