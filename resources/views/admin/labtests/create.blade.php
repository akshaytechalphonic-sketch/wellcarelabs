...
@section('title', 'Dashboard - Create Lab Test')

@section('content')
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">

{{-- External CSS --}}
<link href="{{ asset('Back_end/css/tests/create-test.css') }}" rel="stylesheet">
...
<div class="admin-page-wrapper">
  <div class="card shadow-sm admin-page-card-flush">
    ...
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2 bg-white border-bottom">
      <div>
        <h5 class="panel-title mb-0">Create Lab Test</h5>
        <small class="text-muted">Add a new lab test</small>
      </div>
    ...
    <div class="card-body card-body-soft">
      ...
      <form action="{{ route('admin.labtests.store') }}" method="POST" id="labTestCreateForm" novalidate>
        @csrf
        ...
        <div class="mb-3">
          ...
          <textarea id="description" name="description" rows="4" required
            ...
            class="ckeditor form-control wc-control @error('description') is-invalid @enderror">{{ old('description') }}</textarea>
          @error('description')
          <div class="invalid-feedback d-block">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="mb-3">
          ...
          <textarea id="meta_tags" name="meta_tags" rows="5"
            ...
            class="form-control wc-control @error('meta_tags') is-invalid @enderror">{{ old('meta_tags') }}</textarea>
          @error('meta_tags')
          <div class="invalid-feedback d-block">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="row">
          ...
          <div class="col-md-4 mb-3">
            ...
            <input id="b2b" type="number" step="0.01" name="b2b"
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <textarea id="why_done" name="why_done" rows="3"
              ...
              placeholder="Enter details...">{{ old('why_done') }}</textarea>
            @error('why_done')
            <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
          </div>
          <div class="col-md-6 mb-3">
            ...
            <textarea id="who_should_test" name="who_should_test" rows="3"
              ...
              placeholder="Enter details...">{{ old('who_should_test') }}</textarea>
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <textarea id="how_to_read" name="how_to_read" rows="3"
              ...
              placeholder="Enter details...">{{ old('how_to_read') }}</textarea>
            @error('how_to_read')
            <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
          </div>
          <div class="col-md-6 mb-3">
            ...
            <textarea id="what_to_ask" name="what_to_ask" rows="3"
              ...
              placeholder="Enter details...">{{ old('what_to_ask') }}</textarea>
            @error('what_to_ask')
            <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
          </div>
        ...
        <div class="mb-3">
          ...
          <div class="wc-select-shell" id="statusDropdown">
            ...
            <select name="status"
              id="statusSelect"
              class="status-select @error('status') is-invalid @enderror"
              required>
              <option value="" disabled {{ $oldStatus ? '' : 'selected' }}>-- Select Status --</option>
...
<script src="{{ asset('Back_end/js/tests/create-test.js') }}"></script>
@endsection