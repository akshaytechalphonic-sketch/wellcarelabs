...
@section('title', 'Dashboard - Create Dynamic Page')
    ...
    <div class="admin-page-wrapper">
        <div class="card admin-page-card-flush">
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2 bg-white border-bottom">
                <div>
                    <h5 class="panel-title mb-0">Create Dynamic Page</h5>
                    <small class="text-muted">Add a new dynamic layout page</small>
                </div>
            ...
            <div class="card-body card-body-soft p-4">
                <form action="{{ route('admin.pages.store') }}" method="POST" id="pageCreateForm" enctype="multipart/form-data" novalidate>
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="meta_description" class="form-control wc-control @error('meta_description') is-invalid @enderror"
                            placeholder="Enter Meta Description for search results" rows="3">{{ old('meta_description') }}</textarea>
                    ...
                    {{-- <div class="mb-3">
                        ...
                        <textarea name="schema_markup" class="form-control wc-control @error('schema_markup') is-invalid @enderror"
                            placeholder="Enter JSON-LD script for rich search snippets" rows="4">{{ old('schema_markup') }}</textarea>
                        @error('schema_markup')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div> --}}
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="meta_tags" class="form-control wc-control @error('meta_tags') is-invalid @enderror"
                            placeholder="e.g. custom Google Tag Manager, custom CSS, or header code snippets" rows="3">{{ old('meta_tags') }}</textarea>
                        @error('meta_tags')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input name="image_alt" value="{{ old('image_alt') }}"
                                    ...
                                    @foreach($packages as $pkg)
                                        <div class="form-check mb-2 package-check-item">
                                            <input class="form-check-input" type="checkbox" name="package_ids[]" value="{{ $pkg->id }}" id="pkg_{{ $pkg->id }}"
                                                {{ (is_array(old('package_ids')) && in_array($pkg->id, old('package_ids'))) ? 'checked' : '' }}>
                                    ...
                                    @foreach($faqs as $faq)
                                        <div class="form-check mb-2 faq-check-item">
                                            <input class="form-check-input" type="checkbox" name="faq_ids[]" value="{{ $faq->id }}" id="faq_{{ $faq->id }}"
                                                {{ (is_array(old('faq_ids')) && in_array($faq->id, old('faq_ids'))) ? 'checked' : '' }}>
                    ...
                    <div class="mb-4 mt-4 border-top pt-4">
                        ...
                        <div id="pageSectionsContainer" class="d-flex flex-column gap-3">
                            {{-- Start with one default text section --}}
                            <div class="card p-3 border rounded shadow-sm mb-3 section-card" data-section-id="new_1">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <span class="fw-bold text-dark"><i class="fa-solid fa-layer-group me-1"></i> Section 1</span>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        ...
                                        <select name="sections[new_1][type]" class="form-control wc-control section-type-select">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input name="sections[new_1][title]" type="text" class="form-control wc-control" value="Main Content">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="wc-field-label">Sort Order</label>
                                        <input name="sections[new_1][sort_order]" type="number" class="form-control wc-control" value="1">
                                    </div>
                                    <div class="col-md-12 section-subtitle-wrap">
                                        <label class="wc-field-label">Section Subtitle</label>
                                        <input name="sections[new_1][subtitle]" type="text" class="form-control wc-control" value="">
                                    </div>
                                    <div class="col-md-12">
                                        <label class="wc-field-label">Content / Body / HTML Description</label>
                                        <textarea name="sections[new_1][body]" class="form-control wc-control section-body-editor" rows="4"></textarea>
                                    </div>
                                    <div class="col-md-12 section-image-wrap d-none">
                                        <label class="wc-field-label">Section Image</label>
                                        <input name="sections[new_1][image]" type="file" class="form-control wc-control" accept="image/*">
                                    </div>
                                </div>
                            </div>
                        </div>
