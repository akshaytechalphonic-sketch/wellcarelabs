...
@section('title', 'Dashboard - Create Banner')
...
@section('content')
    <style>
        ...
        /* Page spacing around sidebar & topbar (consistent with other admin pages) */
        .admin-page-wrapper {
            margin: 20px 22px 25px 22px;
        }

        /* Flush card styling consistent with other pages */
        ...
        /* === STATUS DROPDOWN (same pattern as lab test) === */
        ...
        /* Native select (kept for form + accessibility, visually hidden) */
        .status-select {
            position: absolute;
            inset: 0;
            opacity: 0;
    ...
    <div class="admin-page-wrapper">
        <div class="card shadow-sm admin-page-card-flush">
            ...
            <div
                ...
                <div>
                    <h5 class="panel-title mb-0">Create Banner</h5>
                    <small class="text-muted">Add a new site hero banner</small>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.banners.index') }}" class="btn btn-ghost-secondary">
            ...
            <div class="card-body card-body-soft">
                ...
                @php
                    $oldStatus = old('is_active'); // "1", "0" or null
                @endphp

                <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data"
                    id="bannerCreateForm" novalidate>
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-7">
                            ...
                            <div class="mb-3">
                                <label class="wc-field-label">
                                    Image (recommended: 1600×600) <span class="text-danger">*</span>
                                    <small class="text-muted"> — jpg, png, webp</small>
                                </label>
                                <input type="file" name="image" id="bannerImageInput"
                                    class="form-control wc-control @error('image') is-invalid @enderror" accept="image/*"
                                    required>
                                @error('image')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                <div class="wc-help-text mt-1">
                                    Large, wide images work best for the home hero slider.
                                </div>
                            </div>

                            {{-- Status (custom dropdown like lab test) --}}
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="wc-select-shell" id="isActiveDropdown">
                                        ...
                                        <select name="is_active" id="isActiveSelect"
                                            class="status-select @error('is_active') is-invalid @enderror" required>
                                            <option value="" disabled
                                                {{ $oldStatus === null || $oldStatus === '' ? 'selected' : '' }}>
                                                -- Select Status --
                                            </option>
                            ...
                            <div class="d-flex gap-2 mt-3">
                                <button type="submit" id="bannerCreateSubmit" class="btn btn-primary-pill">
                        ...
                        <div class="col-md-5 mt-3 mt-md-0">
                            <div class="mb-2">
                                <span class="wc-field-label d-block" style="font-size:0.9rem;">Live Preview</span>
                                <span class="wc-help-text">Preview of the uploaded image (for reference only).</span>
                            </div>

                            <div id="bannerImagePreview" class="p-3 border rounded-3 bg-white" style="display:none;">
                                <img id="bannerPreviewImg" src="#" alt="Preview">
                            </div>

                            <div id="bannerPreviewPlaceholder" class="p-4 border rounded-3 bg-white text-center">
                                No image selected yet. Choose an image to see a preview here.
                            </div>
                            ...
                            <div class="mt-3">
                                ...
                                <input type="url" name="url" id="url" class="form-control"
                                    placeholder="https://example.com/page" value="{{ old('url', $banner->url ?? '') }}">
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('bannerCreateForm');
            const submitBtn = document.getElementById('bannerCreateSubmit');
            const fileInput = document.getElementById('bannerImageInput');
            const previewWrap = document.getElementById('bannerImagePreview');
            ...
            if (form && submitBtn) {
                form.addEventListener('submit', function(e) {
                    ...
                    // Safety fallback: re-enable after 3s if validation fails
                    setTimeout(() => {
                        if (submitBtn) {
                            submitBtn.disabled = false;
                            submitBtn.innerHTML =
                                '<i class="fa-solid fa-check"></i><span>Create Banner</span>';
                        }
                    }, 3000);
                });
            }

            // ===== Custom status dropdown (same logic as lab test) =====
            ...
            // Initialize active item based on old('is_active')
            ...
            if (statusShell && statusMenu) {
                ...
                // Keyboard shortcuts A / I when shell focused
                statusShell.addEventListener('keydown', function(e) {
                    if (e.key === 'Escape') {
                        closeMenu();
                        statusShell.blur();
                    }
                    if (e.key.toLowerCase() === 'a') { // Active
                        setStatusValue('1');
                        closeMenu();
                    }
                    if (e.key.toLowerCase() === 'i') { // Inactive
                        setStatusValue('0');
                        closeMenu();
                    }
